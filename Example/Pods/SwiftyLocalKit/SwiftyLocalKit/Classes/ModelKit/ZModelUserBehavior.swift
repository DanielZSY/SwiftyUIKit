...
public class ZModelUserBehavior: ZModelBase {
    ...
    public var behavior_user_local: ZModelUserLocal?
    public var behavior_userid: String = ""
    public var behavior_time: Double = Date().timeIntervalSince1970
    public var behavior_type: ZEnumUserBehaviorType = .none
    ...
    public required init<T: ZModelBase>(instance: T) {
        ...
        if let user = model.behavior_user_local {
            self.behavior_user_local = ZModelUserLocal.init(instance: user)
        }
    ...
    public required init(row: Row) {
        ...
        if let _ = row["userid"] {
            if ZSettingKit.shared.ismainapp { self.behavior_user = ZModelUserBase.init(row: row) }
            else { self.behavior_user_local = ZModelUserLocal.init(row: row) }
        }
