...
public class ZSQLiteKit: NSObject {
    ...
    // 数据库地址
    private let dbPath = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.cachesDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).first?.appendingPathComponent("DBDatabase.sqlite") ?? "DBDatabase.sqlite"
    /// 连接数据库
