    ...
    static func createTableVersion() {
        do {
            try ZSQLiteKit.shared.connection?.write { db in
                ...
                try db.create(table: ZModelUserLocal.databaseTableName, ifNotExists: true, body: { t in
                    ...
                    t.column("role", .integer).notNull().defaults(to: ZEnumUserRole.user)
                    t.column("gender", .integer).notNull().defaults(to: ZEnumUserGender.male)
                    ...
                    t.column("is_follow", .text).notNull().defaults(to: false)
                    t.column("is_black", .text).notNull().defaults(to: false)
                    t.primaryKey(["userid"])
                })
                try db.create(table: ZModelMessageRecord.databaseTableName, ifNotExists: true, body: { t in
                    t.column("message_userid", .text).notNull().defaults(to: "").indexed()
                    t.column("message_id", .text).notNull().defaults(to: "")
                    t.column("login_userid", .text).notNull().defaults(to: "").indexed()
                    t.primaryKey(["message_userid", "message_id", "login_userid"])
                })
                try db.create(table: ZModelMessage.databaseTableName, ifNotExists: true, body: { t in
                    ...
                    t.column("message_type", .integer).notNull().defaults(to: ZEnumMessageType.text)
                    t.column("message_time", .double).notNull().defaults(to: 0)
                    t.column("message_direction", .integer).notNull().defaults(to: ZEnumMessageDirection.send)
