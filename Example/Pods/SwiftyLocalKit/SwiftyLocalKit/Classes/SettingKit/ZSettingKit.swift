...
public class ZSettingKit: NSObject {
    ...
    /// 用户对象
    public var userLogin: ZModelUserLocal?  {
    ...
    public var videos: [String]? {
        return self.getLoginUser(key: kLoginUserVideosKey)?["videos"] as? [String]
    }
    /// 是否登录
    ...
    private var modelUserLogin: ZModelUserLocal?
    private var apitoken: String = ""
    internal var ismainapp: Bool = true
    ...
    public final func reloadUser(_ isapp: Bool = true) {
        ...
        if let dic = self.getLoginUser() {
            if self.ismainapp {
                ...
                self.modelUserLogin = ZModelUserLocal.deserialize(from: dic)
    ...
    public final func updateVideos(array: [String]) {
        if array.count > 0 {
            ...
            dic["videos"] = array
            dic["time"] = Date().timeIntervalSince1970
            self.setLoginUser(dic: dic, key: kLoginUserVideosKey)
        } else {
            UserDefaults.standard.removeObject(forKey: kLoginUserVideosKey)
        }
    }
